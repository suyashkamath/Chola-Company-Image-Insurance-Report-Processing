...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://chola-image-report.vercel.app"],  # Your frontend
...
location has 2 columns here , so while writing location please write Zone:Region as table is given with two columns for location.
Product is also given , Product and Sub category column are given , so it is notthing but the segment
for example product column has value LCV and Sub category has value 2.5-3.5T , it is nothing but segment ALL GVW & PCV 3W, GCV 3W
and in the input file policy type is given as Segment column and there is one column named New PO% , so please consider it as Payin value , ignore rest column please 
"""
